...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String originalCurrency = "USD"; // Örnek olarak USD
  List<String> targetCurrencies = [
    "EUR",
    "GBP",
    "JPY"
  ]; // Örnek olarak EUR, GBP, JPY
  double rate = 0;
  double rate2 = 0;
...
// ...
  ...
  Future<void> initState() async {
    ...
    for (String targetCurrency in targetCurrencies) {
      // API isteği oluşturun ve döviz kuru bilgisini alın
      CurrencyResponseModel response = await getCurrencyConversionRate(
        convertFrom: originalCurrency,
        convertTo: targetCurrency,
        amount: 20,
      );

      // Döviz kuru bilgisini kullanarak miktarı dönüştürün
      double convertedAmount = 20 * response.result!;

      // Sonucu depolayın
      convertedAmounts[targetCurrency] = convertedAmount;
    }

// Kullanıcıya dönüştürülmüş miktarları gösterin
    print("Döviz Çevrim Sonuçları:");
    for (String targetCurrency in targetCurrencies) {
      print(
          "20 $originalCurrency = ${convertedAmounts[targetCurrency]} $targetCurrency");
    }
    ...
    void initState() async {
      ...
      Future<void> _initializeData() async {
        // Eşzamansız işlemleri burada yapabilirsiniz
        // Örneğin, API çağrıları, veri yükleme, vb.

        // Örneğin, döviz kurlarını yükleme işlemini burada yapabilirsiniz
        for (String targetCurrency in targetCurrencies) {
          try {
            // API çağrısı ve işlemleri burada yapabilirsiniz
            CurrencyResponseModel response = await getCurrencyConversionRate(
              convertFrom: originalCurrency,
              convertTo: targetCurrency,
              amount: 20,
